...
class TestDataClass(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertIn(
            InputRepresentationName, self.data_obj._processed_representations.nodes
        )
        self.assertIn(
            OutputRepresentationName, self.data_obj._processed_representations.nodes
        )
    ...
    def test_apply_filter(self):
        ...
        filter_obj = MockFilter(
            input_is_chunked=True, name=filter_name, operation=double_data
        )

        # Apply the filter
        result_name = self.data_obj.apply_filter(
            filter=filter_obj, representations_to_filter=[InputRepresentationName]
        )
        ...
        output_name = self.data_obj.apply_filter(
            filter=output_filter, representations_to_filter=[filter_name]
        )
        ...
        discard_filter = MockFilter(
            input_is_chunked=True, name="discard_test", operation=lambda x: x * 4
        )
    ...
    def test_apply_filter_sequence(self):
        ...
        filter1 = MockFilter(
            input_is_chunked=True, name="double", operation=double_data
        )

        filter2 = MockFilter(input_is_chunked=True, name="add_one", operation=add_one)

        filter3 = MockFilter(
            input_is_chunked=True, name="square", operation=square_data
        )
        ...
        with self.assertRaises(ValueError):
            self.data_obj.apply_filter_sequence(
                filter_sequence=[], representations_to_filter=[InputRepresentationName]
            )
        ...
        new_filter = MockFilter(
            input_is_chunked=True, name="final", operation=lambda x: x / 2
        )
    ...
    def test_apply_filter_pipeline(self):
        ...
        branch1_filter1 = MockFilter(
            input_is_chunked=True, name="abs", operation=abs_data
        )
        branch1_filter2 = MockFilter(
            input_is_chunked=True, name="log", operation=log_data
        )
    ...
    def test_getitem_and_deleted_representation(self):
        ...
        # Create a filter
        filter_obj = MockFilter(
            input_is_chunked=True, name="test_filter", operation=double_data
        )
        ...
        np.testing.assert_array_equal(
            self.data_obj[LastRepresentationName], self.data_obj["test_filter"]
        )
    ...
    def test_delete_methods(self):
        ...
        # Create filters
        filter1 = MockFilter(
            input_is_chunked=True, name="filter1", operation=double_data
        )
        filter2 = MockFilter(input_is_chunked=True, name="filter2", operation=add_one)
        ...
        self.assertIn(
            "filter1", self.data_obj._data
        )  # Data is still there as DeletedRepresentation

        # For the delete test, create a new filter
        delete_filter = MockFilter(
            input_is_chunked=True, name="to_delete", operation=square_data
        )
        self.data_obj.apply_filter(
            filter=delete_filter, representations_to_filter=[InputRepresentationName]
        )
        ...
        filter3 = MockFilter(
            input_is_chunked=True, name="filter3", operation=lambda x: x * 3
        )
        self.data_obj.apply_filter(
            filter=filter3, representations_to_filter=[InputRepresentationName]
        )
    ...
    def test_copy(self):
        ...
        filter_obj = MockFilter(
            input_is_chunked=True, name="filter_copy_test", operation=double_data
        )
        ...
        filter_obj2 = MockFilter(
            input_is_chunked=True, name="different_filter", operation=square_data
        )
        data_obj2.apply_filter(
            filter=filter_obj2, representations_to_filter=[InputRepresentationName]
        )
        ...
        self.assertIsNot(
            data_obj._processed_representations, data_obj2._processed_representations
        )
    ...
    def test_save_and_load(self):
        ...
        filter_obj = MockFilter(
            input_is_chunked=True, name="save_test", operation=double_data
        )
        ...
        try:
            ...
            self.assertEqual(
                loaded_data.sampling_frequency, data_obj.sampling_frequency
            )
            ...
            np.testing.assert_array_equal(
                loaded_data["save_test"], data_obj["save_test"]
            )
    ...
    def test_memory_usage(self):
        ...
        filter_obj = MockFilter(
            input_is_chunked=True, name="memory_test", operation=double_data
        )
        self.data_obj.apply_filter(
            filter=filter_obj, representations_to_filter=[InputRepresentationName]
        )
    ...
    def test_get_representation_history(self):
        ...
        # Create a sequence of filters
        filter1 = MockFilter(input_is_chunked=True, name="step1", operation=double_data)
        filter2 = MockFilter(input_is_chunked=True, name="step2", operation=add_one)
        filter3 = MockFilter(input_is_chunked=True, name="step3", operation=square_data)
        ...
        self.assertEqual(history, [InputRepresentationName, "step1", "step2", "step3"])

        # Check history with a branch
        branch_filter = MockFilter(
            input_is_chunked=True, name="branch", operation=lambda x: x / 2
        )
        self.data_obj.apply_filter(
        ...
        self.assertEqual(
            branch_history, [InputRepresentationName, "step1", "step2", "branch"]
        )
