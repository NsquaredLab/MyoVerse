...
class TestKinematicsData(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        with self.assertRaises(ValueError):
            KinematicsData(np.random.randn(16, 100), 100.0)  # 2D array, should be at least 3D
            
        with self.assertRaises(ValueError):
            KinematicsData(np.random.randn(5, 16, 3, 100, 2), 100.0)  # 5D array, should be at most 4D
    ...
    def test_plot(self, mock_plot):
        ...
        mock_plot.assert_called_once_with("Input", nr_of_fingers=5, wrist_included=False)
        mock_plot.reset_mock()
